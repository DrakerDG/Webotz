...
// 0.12, 0.13, 0.14, 0.18, 0.20, 0.27
// 0.30+, 0.31+, 0.33, 0.37, 0.39+
float Kp=0.31;
float Ki=0.00; //0.00
float Kd=0.01; //0.01, 0.02


#define LFM_FORWARD_SPEED 200 //200
  ...
  for (;;) {
    ...
    if(ontrack){
      ...
      if(online){
        ...
        if(gs_new[0]<100 && gs_new[1]>800 && gs_new[2]<100){
...
//      Counts how many times the line is detected with the right sensor  
        if(P>OL_GS) lineR++;
//      Counts how many times the line is detected with the all sensors 
        if(gs_new[0]>800 && gs_new[1]>800 && gs_new[2]>800) AllGS++;
      }
...
//    printf("%4d   %4d   %4d   %5d   OnLine: %d   LineL: %2d   LineR: %2d   AllGS %2d   Left: %d   Right: %d  \n", gs_new[0], gs_new[1], gs_new[2], (int)P, online, lineL, lineR, AllGS, turnL, turnR);
...
//  The robot rotates until the ps5 sensor stops detecting the obstacle     
    if(recovery){
      ...
      else if(gs_new[0]>800){ //|| gs_new[1]>800 || gs_new[2]>800){
        speed[LEFT] = 200;
        speed[RIGHT] = 200;
      }
      else{
...
//    printf("%4d   %4d   %4d   %4d   %5d   OnLine: %d  \n", gs_new[0], gs_new[1], gs_new[2], (int)Position, (int)P, online);
